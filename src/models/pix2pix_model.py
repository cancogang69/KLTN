...
class Pix2PixModel(BaseModel):
    ...
    def set_input(self, input, target):
        ...
        self.real_A = torch.Tensor(input).unsqueeze(0).unsqueeze(0).to(self.device)
        self.real_B = torch.Tensor(target).unsqueeze(0).unsqueeze(0).to(self.device)

    def forward(self):
        """Run forward pass; called by both functions <optimize_parameters> and <test>."""
        torch_resize = Resize(size=self.target_shape)
        self.fake_B = torch_resize(self.netG(self.real_A)).to(self.device) # G(A)
